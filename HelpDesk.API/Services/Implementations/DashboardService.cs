...
namespace HelpDesk.API.Services.Implementations
{
     public class DashboardService : IDashboardService
     {
          ...
          public async Task<DashboardDto> GetDashboard()
          {
               ...
               {
                    OpenTickets =
                       statusCounts.FirstOrDefault(
                           s => s.StatusId == openStatusId)?.Count ?? 0,

                    InProgressTickets =
                       statusCounts.FirstOrDefault(
                           s => s.StatusId == progressStatusId)?.Count ?? 0,

                    ResolvedTickets =
                       statusCounts.FirstOrDefault(
                           s => s.StatusId == resolvedStatusId)?.Count ?? 0,

                    ClosedTickets =
                       statusCounts.FirstOrDefault(
                           s => s.StatusId == closedStatusId)?.Count ?? 0,
